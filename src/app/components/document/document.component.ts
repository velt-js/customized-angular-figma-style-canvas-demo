...
export class DocumentComponent implements AfterViewInit {
	...
	private allCommentAnnotations: any[] = [];
	private currentCommentAnnotationIndex: any;
	// Getting Velt Client
		...
		effect(() => {
			...
			if (this.client) {
				...
				commentElement.getAllCommentAnnotations().subscribe((commentAnnotations: any) => {
					this.allCommentAnnotations = commentAnnotations;
				});

				commentElement.onCommentSelectionChange().subscribe((event: any) => {
					this.currentCommentAnnotationIndex = event.annotation.annotationIndex;
				});
			}
		});
	...
	getPreviousComment = () => {
		console.log(this.allCommentAnnotations);
		console.log(this.currentCommentAnnotationIndex);

		let index = this.currentCommentAnnotationIndex - 1

		if (index < 0) {
			index = this.allCommentAnnotations.length - 1;
		} else if (index >= this.allCommentAnnotations.length) {
			index = 0;
		}

		console.log(this.allCommentAnnotations[index].annotationId);

		this.client?.getCommentElement()?.selectCommentByAnnotationId(this.allCommentAnnotations[this.currentCommentAnnotationIndex + 1].annotationId);
	}
	getNextComment = () => {
		console.log(this.allCommentAnnotations);
		console.log(this.currentCommentAnnotationIndex);
